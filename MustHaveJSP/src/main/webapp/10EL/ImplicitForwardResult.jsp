...
<body>
	...
	<!-- 
		forward되면 page영역은 소멸되고 request영역은 공유되므로 
		아래에서 page영역 부분은 아무값도 출력되지 않는다.
	-->
	<ul>
		<!-- 출력한 값이 null이면 아무것도 출력되지 않는다. -->
	...
	<!-- 
		앞의 Main 파일을 한번 실행하면 4가지 영역 전체에 속성값을 저장하므로
		session, application영역에도 저장된다.
		이 값들은 브라우저를 종료하거나 혹은 서버를 종료해야 소멸되는 영역
		이므로 해당 파일을 단독으로 실행하더라도값이 출력된다.
